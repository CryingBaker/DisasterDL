...
def run_training(model, train_loader, val_loader, criterion,
                 epochs, optimizer, scheduler, patience=12, debug=False,
                 start_epoch=1, best_iou=-1.0, prefix=""):
    
    os.makedirs("logs",   exist_ok=True)
    os.makedirs("models", exist_ok=True)

    # Add the prefix to the file paths
    best_pt  = f"models/{prefix}best_model.pth"
    last_pt  = f"models/{prefix}last_model.pth"
    log_csv  = f"logs/{prefix}training_log.csv"
    txt_path = Path(f"logs/{prefix}training_metrics.txt")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        print(
            f"[Train][{epoch:03d}/{epochs}] LR:{lr:.5f} | "
    ...
    print(f"\nTraining done. Best Val Flood IoU: {best_flood_iou:.4f}")
...
def main(debug=False, resume=False):
    ...
    epochs     = 1 if debug else 60

    print("\n" + "="*60)
    print("TRAINING (Hand-Labeled Data Only)")
    print("="*60)

    tr_loader, vl_loader, _ = get_dataloaders(".", batch_size=batch_size)
    model = build_model(20, 2).to(device)
    ...
    if resume and os.path.exists("models/last_model.pth"):
        ...
        best_iou    = ckpt.get('best_val_iou_flood', -1.0)
        print(f"Resuming training from epoch {start_epoch} (best IoU: {best_iou:.4f})")

    enc_params = list(model.encoder.parameters())
    dec_params = [p for n, p in model.named_parameters() if not n.startswith('encoder')]
    
    # Slightly unified learning rate for training from scratch on hand labels
    opt = torch.optim.AdamW([
        {'params': enc_params, 'lr': 5e-5}, 
        {'params': dec_params, 'lr': 1e-4}, 
    ], weight_decay=1e-4)
    sch = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=epochs, eta_min=1e-6)

    if resume and os.path.exists("models/last_model.pth") and start_epoch > 1:
        ckpt = torch.load("models/last_model.pth", map_location=device)
        opt.load_state_dict(ckpt['optimizer_state_dict'])
        sch.load_state_dict(ckpt['scheduler_state_dict'])

    run_training(
        model, tr_loader, vl_loader, criterion,
